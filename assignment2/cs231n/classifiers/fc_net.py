...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        self.params['W2'] = weight_scale * np.random.randn(hidden_dim, output_size)
    ...
    def loss(self, X, y=None):
        ...
        w2 = self.params['W2'] 
        b2 = self.params['b2'] 
        out_affine1, cache_af1 = affine_forward(X, w1, b1)
        out_relu, cache_r = relu_forward(out_affine1)
        out_affine2, cache_af2 = affine_forward(out_relu, w2, b2)
        scores = out_affine2
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        loss, dout_af2 = softmax_loss(scores, y)
        loss += self.reg * 0.5 * np.sum(w1 * w1) + self.reg * 0.5 * np.sum(w2 * w2)
        dout_r = affine_backward(dout_af2, cache_af2)[0]
        dout_af1 = relu_backward(dout_r, cache_r)
        dw1 = affine_backward(dout_af1, cache_af1)[1] + 2*self.reg * 0.5 * w1
        db1 = affine_backward(dout_af1, cache_af1)[2]
        dw2 = affine_backward(dout_af2, cache_af2)[1] + 2*self.reg * 0.5 * w2
        db2 = affine_backward(dout_af2, cache_af2)[2]
        grads['W1'] = dw1
        grads['W2'] = dw2
        grads['b1'] = db1
        grads['b2'] = db2
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        self.params['W'+str(num_hidden_layers + 1)] = weight_scale * np.random.randn(hidden_dims[-1], output_size)
        self.params['b'+str(num_hidden_layers + 1)] = np.zeros(output_size)
    ...
    def loss(self, X, y=None):
        ...
        if self.normalization == 'batchnorm':
            ...
            out_last, cache_last = affine_forward(out, w, b)
            scores = out_last
        ...
        if self.normalization == 'batchnorm':
            ...
            #Last Layer
            dout, grads['W'+str(num_layers)], grads['b'+str(num_layers)]  = affine_backward(dout, cache_last)
            grads['W'+str(num_layers)] += 2*self.reg * 0.5 * self.params['W'+str(num_layers)]
